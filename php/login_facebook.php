...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $facebookID = trim($data['facebookID']); // Password is user googleID
    ...
    if ($result->num_rows > 0) {
        ...
        if (empty($user['social_fb_id'])) {
            $update_fb_id = $conn->prepare('UPDATE `users` SET `social_fb_id` = ? WHERE user_email = ?');
            $update_fb_id->bind_param('ss', $facebookID, $email);
            $update_fb_id->execute();
            $update_fb_id->close();
        ...
        $insert = $conn->prepare('INSERT INTO users (user_email, user_firstname, user_lastname, user_username, user_creation_date,social_fb_id ) VALUES (?, ?, ?, ?, ?, ?)');
        $insert->bind_param('ssssss', $email, $firstName, $lastName, $userName, $date, $facebookID);
